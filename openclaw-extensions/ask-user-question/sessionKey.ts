const LEGACY_POPIAI_SESSION_PREFIX = 'popiai:';
const AGENT_SESSION_PREFIX = 'agent:';
const POPIAI_SESSION_MARKER = 'popiai';
...
export function isAskUserQuestionCandidateSessionKey(sessionKey: string | undefined | null): boolean {
  ...
  if (raw.startsWith(LEGACY_POPIAI_SESSION_PREFIX)) {
    return raw.slice(LEGACY_POPIAI_SESSION_PREFIX.length).trim().length > 0;
  }
  ...
  return source === POPIAI_SESSION_MARKER || source === SUBAGENT_SESSION_MARKER;
}
