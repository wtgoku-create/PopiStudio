...
export const BrowserAnnotationTheme = {
  Light: 'light',
  Dark: 'dark',
} as const;
export type BrowserAnnotationTheme = typeof BrowserAnnotationTheme[keyof typeof BrowserAnnotationTheme];
...
export interface BrowserAnnotationScreenshotRef {
  ...
  capturedAt: number;
  /** Embedded preview for persisted message badges; runtime image transport still uses attachments. */
...
export interface CoworkBrowserAnnotationBatch {
  version: 1;
  id: string;
  /** Draft key used as the storage scope for local screenshot assets. */
  assetDraftKey?: string;
  browserTabId: string;
  documentId: string;
