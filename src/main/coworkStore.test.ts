...
function insertSession(id: string, agentId = 'main', updatedAt = Date.now()): void {
  const now = Date.now();
  db.prepare(
    `INSERT INTO cowork_sessions (id, title, claude_session_id, status, pinned, pin_order, cwd, system_prompt, execution_mode, active_skill_ids, agent_id, mode, selected_agent_ids, created_at, updated_at)
     VALUES (?, 'test', NULL, 'idle', 0, NULL, '/tmp', '', 'local', '[]', ?, 'single', json_array(?), ?, ?)`,
  ).run(id, agentId, agentId, now, updatedAt);
}
...
test('createSession stores multi-agent mode and selected agents', () => {
  const session = store.createSession(
    ...
    '',
    'multi',
    ['main', 'writer', 'reviewer'],
  );

  const loaded = store.getSession(session.id);
  expect(loaded?.mode).toBe('multi');
  expect(loaded?.selectedAgentIds).toEqual(['main', 'writer', 'reviewer']);

  const summary = store.listSessions()[0];
  expect(summary?.mode).toBe('multi');
  expect(summary?.selectedAgentIds).toEqual(['main', 'writer', 'reviewer']);
});

test('getSession falls back to single mode and owner agent for legacy rows', () => {
  const now = Date.now();
  db.prepare(
    `INSERT INTO cowork_sessions (
      id, title, claude_session_id, status, pinned, pin_order, cwd, system_prompt, model_override,
      execution_mode, active_skill_ids, agent_id, mode, selected_agent_ids, created_at, updated_at
    ) VALUES (?, 'legacy', NULL, 'idle', 0, NULL, '/tmp', '', '', 'local', '[]', ?, NULL, NULL, ?, ?)`,
  ).run('legacy-session', 'legacy-agent', now, now);

  const loaded = store.getSession('legacy-session');
  expect(loaded?.mode).toBe('single');
  expect(loaded?.selectedAgentIds).toEqual(['legacy-agent']);
});
