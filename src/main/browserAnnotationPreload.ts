...
function start(envelope: BrowserAnnotationGuestEnvelope): void {
  ...
  root.dataset.theme = envelope.theme === BrowserAnnotationTheme.Dark
  ...
  style.textContent = `
    :host{
      all:initial;
      --annotation-accent:#1683ff;
      --annotation-panel:#ffffff;
      --annotation-panel-raised:#f7f8fa;
      --annotation-panel-hover:#eef1f5;
      --annotation-border:#d8dde6;
      --annotation-border-strong:#c3cad5;
      --annotation-text:#1f2328;
      --annotation-muted:#6b7280;
      --annotation-placeholder:#9aa3af;
      --annotation-control:#ffffff;
      --annotation-primary:#111827;
      --annotation-primary-text:#ffffff;
      --annotation-shadow:0 18px 54px rgba(15,23,42,.18);
      --annotation-highlight-ring:rgba(255,255,255,.86);
    }
    :host([data-theme="dark"]){
      --annotation-panel:#292929;
      --annotation-panel-raised:#303030;
      --annotation-panel-hover:#3a3a3a;
      --annotation-border:#3b3b3b;
      --annotation-border-strong:#555;
      --annotation-text:#f5f5f5;
      --annotation-muted:#b8b8b8;
      --annotation-placeholder:#a4a4a4;
      --annotation-control:#303030;
      --annotation-primary:#ffffff;
      --annotation-primary-text:#202020;
      --annotation-shadow:0 18px 54px rgba(0,0,0,.48);
      --annotation-highlight-ring:rgba(255,255,255,.82);
    }
    *{box-sizing:border-box;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
    .highlight{position:fixed;display:none;border:2px solid var(--annotation-accent);background:rgba(22,131,255,.12);box-shadow:0 0 0 1px var(--annotation-highlight-ring);pointer-events:none}
    .marker,.pending-marker{position:fixed;width:26px;height:26px;border:2px solid white;border-radius:999px;background:var(--annotation-accent);color:white;font:600 12px/22px sans-serif;text-align:center;box-shadow:0 2px 7px rgba(0,0,0,.3)}
    .marker{pointer-events:auto;cursor:pointer}.pending-marker{display:none;pointer-events:none}
    .editor{position:fixed;display:none;width:352px;max-height:min(380px,calc(100vh - 16px));overflow:hidden;border:1px solid var(--annotation-border);border-radius:18px;background:var(--annotation-panel);color:var(--annotation-text);box-shadow:var(--annotation-shadow);pointer-events:auto}
    .editor-top{display:flex;align-items:center;gap:8px;min-height:58px;padding:9px 10px}
    .icon-button{display:grid;place-items:center;width:34px;height:34px;flex:0 0 34px;border:0;border-radius:999px;background:transparent;color:var(--annotation-muted);cursor:pointer}
    .icon-button:hover,.icon-button.active{background:var(--annotation-panel-hover);color:var(--annotation-text)}.icon-button:disabled{opacity:.35;cursor:default}
    .comment{width:100%;min-width:0;min-height:34px;max-height:92px;resize:none;border:0;background:transparent;color:var(--annotation-text);padding:7px 2px;outline:none;font-size:14px;line-height:20px}
    .comment::placeholder{color:var(--annotation-placeholder)}
    .top-confirm{display:grid;place-items:center;width:34px;height:34px;flex:0 0 34px;border:0;border-radius:999px;background:var(--annotation-primary);color:var(--annotation-primary-text);font-size:18px;cursor:pointer}
    button:disabled{opacity:.42;cursor:default}
    .properties{display:none;max-height:min(272px,calc(100vh - 124px));overflow:auto;border-top:1px solid var(--annotation-border);background:var(--annotation-panel-raised)}
    .editor.expanded .properties{display:block}
    .property-title{display:flex;align-items:center;justify-content:space-between;height:38px;padding:0 16px;border-bottom:1px solid var(--annotation-border);color:var(--annotation-text);font-size:14px}
    .drag-dots{color:var(--annotation-placeholder);letter-spacing:1px}
    ...
    .property-label{color:var(--annotation-muted);font-size:13px}
    .property-control{width:100%;height:32px;border:1px solid var(--annotation-border-strong);border-radius:10px;background:var(--annotation-control);color:var(--annotation-text);padding:0 11px;outline:none;font-size:13px}
    .property-control:focus{border-color:var(--annotation-muted)}.property-control:disabled{opacity:.45}
    .property-separator{grid-column:1/-1;height:1px;background:var(--annotation-border);margin:1px 0}
    .unit-control{display:flex;align-items:center;height:32px;border:1px solid var(--annotation-border-strong);border-radius:10px;overflow:hidden;background:var(--annotation-control)}
    .unit-control:focus-within{border-color:var(--annotation-muted)}.unit-input{width:100%;min-width:0;height:30px;border:0;background:transparent;color:var(--annotation-text);padding:0 4px 0 11px;outline:none;text-align:right;font-size:13px}
    .unit-input::-webkit-inner-spin-button{display:none}.unit-suffix{padding:0 10px 0 5px;color:var(--annotation-muted);font-size:12px}
    .multi-control{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));height:32px;border:1px solid var(--annotation-border-strong);border-radius:10px;overflow:hidden;background:var(--annotation-control)}
    .multi-control.gap-control{grid-template-columns:repeat(2,minmax(0,1fr))}.multi-input{width:100%;min-width:0;border:0;border-right:1px solid var(--annotation-border);background:transparent;color:var(--annotation-text);outline:none;text-align:center;font-size:12px}.multi-input:last-child{border-right:0}.multi-input::-webkit-inner-spin-button{display:none}
    .color-control{display:flex;align-items:center;gap:7px;height:32px;border:1px solid var(--annotation-border-strong);border-radius:10px;padding:0 9px;background:var(--annotation-control)}
    ...
    .color-value{width:100%;min-width:0;border:0;background:transparent;color:var(--annotation-muted);outline:none;font:12px/20px ui-monospace,SFMono-Regular,Menlo,monospace}
    .bottom-actions{display:flex;align-items:center;gap:8px;min-height:50px;padding:8px 10px;border-top:1px solid var(--annotation-border)}
    .editor.create:not(.expanded) .bottom-actions{display:none}.editor.edit .top-confirm,.editor.expanded .top-confirm{display:none}
    .action-button{height:32px;border:1px solid var(--annotation-border-strong);border-radius:999px;background:transparent;color:var(--annotation-text);padding:0 12px;cursor:pointer;font-size:13px}
    .delete-button{display:grid;place-items:center;width:32px;padding:0;border-color:transparent}.delete-button:hover{background:var(--annotation-panel-hover)}.delete-button svg{width:16px;height:16px}
    .action-spacer{flex:1}.save-button{border-color:var(--annotation-primary);background:var(--annotation-primary);color:var(--annotation-primary-text);font-weight:600}
  ...
  const commandListener = (_event: Electron.IpcRendererEvent, command: BrowserAnnotationGuestEnvelope) => {
    ...
    if (command.theme) {
      root.dataset.theme = command.theme;
    }
