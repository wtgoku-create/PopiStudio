...
contextBridge.exposeInMainWorld('electron', {
  ...
  cowork: {
    // Session management
    startSession: (options: { prompt: string; runtimePrompt?: string; knowledgeBaseIds?: string[]; cwd?: string; systemPrompt?: string; title?: string; activeSkillIds?: string[]; agentId?: string; modelOverride?: string; imageAttachments?: Array<{ name: string; mimeType: string; base64Data: string }> }) =>
      ipcRenderer.invoke('cowork:session:start', options),
    continueSession: (options: { sessionId: string; prompt: string; runtimePrompt?: string; knowledgeBaseIds?: string[]; systemPrompt?: string; activeSkillIds?: string[]; imageAttachments?: Array<{ name: string; mimeType: string; base64Data: string }> }) =>
