...
contextBridge.exposeInMainWorld('electron', {
  ...
  skills: {
    ...
    ) => ipcRenderer.invoke('skills:download', { source, metadata }),
    upgrade: (
      skillId: string,
      downloadUrl: string,
      metadata?: { official: true; category: string },
    ) => ipcRenderer.invoke('skills:upgrade', { skillId, downloadUrl, metadata }),
  ...
  cowork: {
    // Session management
    startSession: (options: { prompt: string; promptDocument?: CoworkPromptDocument; knowledgeBases?: Array<{ id: string; name: string }>; knowledgeFiles?: Array<{ id: string; title: string; knowledgeBaseName?: string; fileType?: string }>; selectedTextSnippets?: Array<{ id: string; text: string; sourceMessageId?: string; sourceMessageType?: 'assistant' | 'artifact_markdown' | 'artifact_text'; sourceId?: string; sourceType?: 'assistant' | 'artifact_markdown' | 'artifact_text'; sourceTitle?: string; sourcePath?: string; artifactId?: string; createdAt: number; startOffset?: number; endOffset?: number }>; cwd?: string; systemPrompt?: string; title?: string; activeSkillIds?: string[]; agentId?: string; modelOverride?: string; imageAttachments?: Array<{ name: string; mimeType: string; base64Data: string }>; planControl?: PlanControl }) =>
      ipcRenderer.invoke('cowork:session:start', options),
    continueSession: (options: { sessionId: string; prompt: string; promptDocument?: CoworkPromptDocument; knowledgeBases?: Array<{ id: string; name: string }>; knowledgeFiles?: Array<{ id: string; title: string; knowledgeBaseName?: string; fileType?: string }>; selectedTextSnippets?: Array<{ id: string; text: string; sourceMessageId?: string; sourceMessageType?: 'assistant' | 'artifact_markdown' | 'artifact_text'; sourceId?: string; sourceType?: 'assistant' | 'artifact_markdown' | 'artifact_text'; sourceTitle?: string; sourcePath?: string; artifactId?: string; createdAt: number; startOffset?: number; endOffset?: number }>; systemPrompt?: string; turnInstructions?: string; activeSkillIds?: string[]; imageAttachments?: Array<{ name: string; mimeType: string; base64Data?: string }>; planControl?: PlanControl }) =>
    ...
    forkSession: (options: { sessionId: string; forkedFromMessageId?: string | null }) =>
    ...
    getPlanModeState: (sessionId: string) => ipcRenderer.invoke('cowork:plan:state', sessionId),
    controlPlanMode: (options: { sessionId: string; control: PlanControl }) =>
      ipcRenderer.invoke('cowork:plan:control', options),
  ...
  auth: {
    ...
    getKnowledgeToken: () => ipcRenderer.invoke('auth:getKnowledgeToken'),
