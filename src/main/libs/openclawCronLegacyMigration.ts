...
// JSON/JSONL files are read solely by the doctor migration.
...
function resolveDoctorConfigPath(stateDir: string): string {
  return path.join(stateDir, '.popiai-cron-doctor-openclaw.json');
}
...
export async function migrateLegacyCronStorageWithDoctor(params: {
  ...
  if (!fs.existsSync(openclawCliPath)) {
    console.warn(`[OpenClaw] Legacy cron storage was detected but OpenClaw CLI is missing at ${openclawCliPath}.`);
  ...
  console.log(`[OpenClaw] Legacy cron storage was detected; running official doctor migration with args ${JSON.stringify(args.slice(1))}.`);
  try {
    ...
    if (result.code === 0) {
      ...
      if (archived.length > 0) {
        console.log(
          '[OpenClaw] Legacy cron doctor migration completed and archived '
