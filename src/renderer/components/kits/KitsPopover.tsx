...
const KitsPopover: React.FC<KitsPopoverProps> = ({
  ...
  const shouldUseInstallGuideLayout = installedKitIds.length === 0;
  ...
  return (
    <div
      ...
      className={`absolute bottom-full left-0 z-50 mb-1 max-w-[calc(100vw-2rem)] overflow-hidden rounded-xl border border-border bg-surface shadow-popover ${
        shouldUseInstallGuideLayout ? 'w-60' : 'w-80'
      }`}
      role="menu"
    >
      {shouldShowSearch && (
      ...
      {/* Kits list */}
      <div
        className={shouldUseInstallGuideLayout ? 'px-4 py-5' : 'overflow-y-auto px-2 py-1.5'}
        style={shouldUseInstallGuideLayout ? undefined : { maxHeight: `${maxListHeight}px` }}
      >
        {isLoading ? (
          <div className={shouldUseInstallGuideLayout
            ? 'flex min-h-[168px] items-center justify-center text-center text-[13px] text-secondary'
            : 'px-3 py-5 text-center text-[13px] text-secondary'
          }>
