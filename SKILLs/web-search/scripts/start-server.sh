...
resolve_node_runtime() {
  ...
  if [ -n "${POPIAI_ELECTRON_PATH:-}" ] && [ -x "${POPIAI_ELECTRON_PATH}" ]; then
    NODE_CMD="$POPIAI_ELECTRON_PATH"
    NODE_ARGS=()
...
ensure_npm_available() {
  ...
  echo "  Please reinstall the web-search skill runtime from Popiai."
  return 1
}
...
if ! resolve_node_runtime; then
  ...
  echo "  Please install Node.js, or run from Popiai so scripts can use Electron runtime."
  exit 1
fi
