...
- After every `popitv__edit_canvas` call that adds or moves nodes, immediately call `popitv__measure_nodes` for the affected node ids before adding or moving any more nodes.
- For multi-node layout work, add and measure incrementally. Do not create a large batch of nodes before measuring; place one logical node group, measure it, beautify its position, confirm it does not overlap, then place the next.
- If measurement shows any overlap, fix the layout with another `popitv__edit_canvas` call before continuing. Do not run the workflow or report completion while overlap remains.
...
4. Immediately measure the newly added or moved node ids with `popitv__measure_nodes`.
5. Compare measured rectangles against all existing measured rectangles. If any overlap or spacing violation exists, move the offending nodes with `popitv__edit_canvas` and measure again.
...
Coordinates are node top-left positions. Never place two nodes at the same `position`, and never reuse a position from an existing node. Keep at least 80 px horizontal and vertical clearance between node rectangles:

- Horizontal clearance: next node x should be at least `previous.x + previous.width + 80`.
- Vertical clearance in the same stage column: next node y should be at least `previous.y + previous.height + 80`.
...
- Before finishing any canvas edit task, perform a final `popitv__measure_nodes` pass for all relevant nodes and correct any remaining overlap.
...
After each add, call `popitv__measure_nodes` with the ids that were just created, then use those measured `{ id, x, y, width, height }` values and the latest canvas snapshot to decide the next node position. This is mandatory for multi-stage canvases so nodes do not stack on top of each other. If the measured node overlaps any other node, immediately move it to the next free position and measure again.
...
Leave at least one node height plus about 80 px vertical spacing between rows. If you omit `position`, the Popiai bridge will assign a stage-aware default position using PopiTV node dimensions and avoid existing nodes when a live canvas snapshot is available. When you do provide `position`, calculate it from the table or measured sizes; do not repeat coordinates across nodes.
