...
此前 thinking 内容已经有独立消息生命周期，`beforeMessageId` 链路可以让单个
thinking block 显示在普通 assistant 回复之前。

OpenClaw 的实时事件不能始终稳定表达多轮工具调用之间的 thinking 边界时，旧实现会把当前
轮次的 thinking 内容合并为一个消息。其结果是：
...
当前 OpenClaw 的 `chat.history` 已保留 assistant 消息中的结构化 thinking 与 tool call 边界；
会话设置为 `reasoningLevel: 'stream'` 时，支持的模型还可以产生 `agent stream=thinking`
增量事件。因此 PopiStudio 采用“实时流用于尽早展示、结构化 history 用于确定顺序和最终对账”
的双通道方案。
...
**Then** PopiStudio 从 history 提取当前用户轮次的 thinking，并按 assistant/tool 边界插入到
相应工具或最终回复之前。
...
对 PopiStudio 管理的 OpenClaw 会话应用模型覆盖时，同时写入
`reasoningLevel: 'stream'`。不支持 thinking 的模型可以忽略该设置，PopiStudio 必须继续支持
history-only 路径。
...
- `openclawRuntimeAdapter.ts`：负责 OpenClaw 事件、history 请求时机、当前活动轮次状态和 IPC 事件。

### 4.3 重复问题根因与修复

工具边界增量同步可能已经为前几个 thinking block 创建了带稳定 key 的消息；最终对账如果只依赖
`CoworkStore.getSession(sessionId)` 返回的最近消息窗口，就会在长工具链结束时看不到早期
thinking，并误判它们不存在。
...
| `src/main/libs/agentEngine/openclawRuntimeAdapter.ts`       | OpenClaw 事件路由和 thinking history 同步      |
