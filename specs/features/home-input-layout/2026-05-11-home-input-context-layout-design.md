...
日期：2026-05-11  
分类：功能  
参考：用户提供的 Codex 输入框参考图与当前 Popiai 首页截图  
...
当前 Popiai 首页里，模型选择器位于页面顶部 header 左侧；文件夹选择器位于大输入框内部左下；当前选中的 Agent 没有在输入区附近显式展示。用户希望首页输入区更接近参考图的信息层级：
