...
popiai 需要支持渠道包投放统计。不同投放渠道在打包或开发启动时传入不同 `keyfrom`，应用安装并启动后需要把渠道来源稳定记录到本地，并在必要的服务端接口中附带该来源，为后续登录、模型、用户信息、支付、埋点和报表统计提供基础数据。
...
- `/openapi/get/luna/hardware/popiai/{env}/update`
- `/openapi/get/luna/hardware/popiai/{env}/update-manual`
...
```text
Mac Intel:
popiai-darwin-x64-<version>-<keyfrom>.dmg

Mac Apple Silicon:
popiai-darwin-arm64-<version>-<keyfrom>.dmg

Windows x64:
popiai-Setup-x64-<version>-<keyfrom>.exe
```

示例：

```text
popiai-darwin-x64-2026.5.14-bilibili.dmg
popiai-darwin-arm64-2026.5.14-bilibili.dmg
popiai-Setup-x64-2026.5.14-bilibili.exe
```
...
| `/openapi/get/luna/hardware/popiai/{env}/update`        | `GET`        | URL query 追加 `firstKeyfrom`、`latestKeyfrom`、`uuid`、`version`、`userId` |
| `/openapi/get/luna/hardware/popiai/{env}/update-manual` | `GET`        | URL query 追加 `firstKeyfrom`、`latestKeyfrom`、`uuid`、`version`、`userId` |
...
/openapi/get/luna/hardware/popiai/prod/update?uuid=...&userId=...&version=...&firstKeyfrom=bilibili&latestKeyfrom=baidu
/openapi/get/luna/hardware/popiai/prod/update-manual?uuid=...&userId=...&version=...&firstKeyfrom=bilibili&latestKeyfrom=baidu
```
...
4. `registerProxyTokenRefresher('popiai-server')` 代理刷新。
...
| 环境   | Base URL                                    |
| ------ | ------------------------------------------- |
| 测试服 | `https://popiai-server.inner.youdao.com` |
| 正式服 | `https://popiai-server.youdao.com`       |
...
- 自动更新检查：`getUpdateCheckUrl()`，正式服为 `/popiai/prod/update`，测试服为 `/popiai/test/update`。
- 手动更新检查：`getManualUpdateCheckUrl()`，正式服为 `/popiai/prod/update-manual`，测试服为 `/popiai/test/update-manual`。
...
16. `/popiai/{env}/update` 和 `/popiai/{env}/update-manual` 保持 GET，只在 query 追加可用的归因参数。
...
1. 双击 `.dmg` 安装包。
2. 在弹出的挂载窗口中找到 `popiai.app`。
3. 右键 `popiai.app`，选择“显示包内容”。
...
1. 双击 `.exe` 安装包完成安装。
2. 打开资源管理器。
3. 在地址栏输入 `%LOCALAPPDATA%\Programs\popiai\resources\keyfrom`。
