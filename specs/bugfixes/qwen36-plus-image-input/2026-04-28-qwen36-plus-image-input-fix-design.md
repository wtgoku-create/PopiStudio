...
1. `popiai-server/qwen3.6-plus-YoudaoInner` 支持图片输入时，OpenClaw 配置必须写出 `input: ['text', 'image']`
...
7. qwen3.6 Plus 任务已经完成后，迟到的 OpenClaw lifecycle 事件不能把 Popiai 会话重新置为 `running`

## 核心结论

**这不是 qwen3.6 Plus 模型本身不支持图片，而是 Popiai 写给 OpenClaw 的模型能力元数据不可靠。**

OpenClaw gateway 依据 `openclaw.json` 中的 `models.providers[*].models[*].input` 判断模型是否支持图片。如果该字段是 `['text']`，gateway 会在请求进入 LLM 前丢弃图片。

**2026-05-06 补充结论：第一次修复只覆盖了 Popiai 写配置的链路，还不够。** 最新日志显示，当前 `openclaw.json` 已经包含 `qwen3.6-plus` / `qwen3.6-plus-YoudaoInner` 的 `input: ['text', 'image']`，但 OpenClaw gateway 仍输出：

```
parseMessageWithAttachments: 1 attachment(s) dropped — model does not support images
provider=popiai-server model=qwen3.6-plus-YoudaoInner
promptImages=0
```

这说明图片不是在 Popiai 侧上传或配置写入阶段丢失，而是在 OpenClaw runtime 的 `resolveGatewayModelSupportsImages()` 预检阶段被判定为不支持图片。根因是 OpenClaw 当前版本的 model catalog 可能没有包含显式配置的 provider model，且 provider 比较使用精确字符串；`qwen-portal` 与 OpenClaw 原生 `qwen` 没有归一化到同一 provider，catalog miss 后函数直接返回 `false`。

**2026-05-06 补充结论 B：qwen3.6 Plus 完成后 UI 仍 loading 不是图片传输问题本身。** 现场唯一 `running` 会话 `82d47ef0-dc99-4d04-99aa-967815af76cb` 的 assistant 消息已写入 `metadata: { isStreaming:false, isFinal:true }`，OpenClaw 日志也显示同一 run 已 `run_completed`。随后 gateway 又发出同一 run 的迟到 `stream=lifecycle, phase=fallback` 事件。Popiai 旧逻辑会在没有 active turn 时把任何非 error agent event 当作 follow-up turn 重新创建，且 `phase=fallback` 没有 handler，最终把本地 session 状态重新置为 `running`，但再也没有 final/end 事件来收尾。

| 场景 | 错误表现 | 根因 |
|---|---|---|
| `popiai-server/qwen3.6-plus-YoudaoInner` | 模型不在 provider 注册表里，图片被丢弃 | `popiai-server` 只注册默认模型，未合并服务端全量模型 |
...
3. **服务端模型按全量列表合并。** `popiai-server` 不再只注册当前默认模型。
...
7. **迟到 lifecycle 事件不能改变已完成状态。** `phase=fallback` 只作为诊断/兼容事件处理，不能创建或重开 Popiai turn；刚关闭的 runId 在短时间内作为 tombstone 保留，防止迟到事件绑定到后续 turn。
...
auth:getModels
  ...
  → openclawConfigSync merges all server models into popiai-server provider
...
`tryPopiaiServerFallback()` 先构造了只包含默认模型的 provider。`openclawConfigSync` 后续看到 `popiai-server` provider 已存在，就跳过了服务端全量模型合并。

#### 修复

1. `tryPopiaiServerFallback()` 使用缓存的服务端全量模型构造 fallback provider
2. `openclawConfigSync` 对 `popiai-server.models` 做 upsert，而不是 `if exists then skip`
...
```typescript
models.providers['popiai-server'].models[] = {
  id: 'qwen3.6-plus-YoudaoInner',
  input: ['text', 'image']
}
```
...
| `src/main/libs/openclawConfigSync.runtime.test.ts` | 回归测试 qwen、custom、popiai-server 的视觉能力配置 |
...
| `src/main/libs/agentEngine/openclawRuntimeAdapter.ts` | Popiai runtime lifecycle 防护：忽略迟到 fallback，tombstone 已关闭 runId |
...
```
provider=popiai-server/qwen3.6-plus-YoudaoInner promptImages=1
```
...
| `popiai-server` 服务端返回多个模型 | OpenClaw provider 包含全部服务端模型 |
...
| `popiai-server/qwen3.6-plus-YoudaoInner` 没进入 catalog 但 config 声明 image | `resolveGatewayModelSupportsImages()` 回退 config，保留图片 |
...
4. `auth:getModels` 失败时，`popiai-server` 只能使用已有缓存或当前模型兜底，无法自动发现新服务端模型。
